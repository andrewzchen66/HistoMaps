          ...
          {commandInfo.isBrief ? (
            <Paper elevation={3} sx={{ maxWidth: "30rem", mx: "auto", py: "0.5rem", my: "0.5rem" }}>
              {typeof commandInfo.output === "string" ? (
                ...
                <TableOutput ariaLabel={"output-"+index} data={commandInfo.output.data} hasHeader={commandInfo.output.hasHeader} />
              )}
            </Paper>
          ) : (
            <Paper elevation={3} sx={{ maxWidth: "30rem", mx: "auto", py: "0.5rem", my: "0.5rem" }}>
              <p aria-label={"command-"+index}>{"Command: " + commandInfo.command}</p>
              {typeof commandInfo.output === "string" ? (
                <p aria-label={"output-"+index}>{"Output: " + commandInfo.output}</p>
              ) : (
                <>
                  <p>Output: </p>
                  <TableOutput ariaLabel={"output-"+index} data={commandInfo.output.data} hasHeader={commandInfo.output.hasHeader} />
